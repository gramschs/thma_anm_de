...
*Welchen Unterschied macht die Dämpfung im Code?* Die DGL wird um den Term
$c\,\dot{x}$ ergänzt. In `f(t, y)` ändert sich nur der zweite Rückgabewert:
statt `-(k/m)*y[0]` steht jetzt `(-k*y[0] - c*y[1]) / m`. Alles andere
bleibt identisch.

Die **Dämpfungszahl** $D = c\,/\,(2\sqrt{km})$ fasst das Verhältnis von
Dämpfer zu Feder dimensionslos zusammen. Für $D < 1$ schwingt das System,
für $D = 1$ ist es der sogenannte **aperiodische Grenzfall** (schnellste
Rückkehr ohne Überschwingen), für $D > 1$ schleicht es in den Ursprung.
...
for D, farbe in zip(D_werte, farben):
    ...
    # Phasenporträt: bei D < 1 spiralförmige Trajektorie,
    #                bei D >= 1 keine Umkreisung des Ursprungs
...
axes[0].set_title('Gedämpfter Schwinger: Zeitsignal')
axes[0].legend()
...
Für $D < 1$ sieht man im Phasenporträt eine einwärts spiralierende Kurve:
Das System schwingt, verliert aber jede Runde Energie an den Dämpfer.
Für $D = 1$ und $D = 2$ gibt es keinen Vorzeichenwechsel der Auslenkung, der
Arm kriecht monoton in den Ursprung. Die gedämpfte **Eigenkreisfrequenz** beträgt
$\omega_d = \omega_0\sqrt{1-D^2}$, was für $D < 1$ eine reelle Zahl ergibt.
