...
* [ ] Sie wissen, was **Konvergenz** in diesem Kontext bedeutet, und können
  eine Konvergenzkurve interpretieren.
```
...
T_start     = 100.0  # Starttemperatur in °C
T_umgebung  =  20.0  # Umgebungstemperatur in °C
...
T_wahr = T_umgebung + (T_start - T_umgebung) * np.exp(-lambda_wahr * t_mess)
...
```{code-cell} python
# --- Modell- und Kostenfunktion definieren ---

def modell(t, lam):
    """Abkühlmodell: T(t) = T_umgebung + (T_start - T_umgebung) * exp(-lam * t).
    T_start und T_umgebung sind bekannte Größen aus dem äußeren Scope."""
    return T_umgebung + (T_start - T_umgebung) * np.exp(-lam * t)

def kosten(lam):
    """Mittlerer quadratischer Fehler (MSE) zwischen Modell und Messdaten.
    Misst, wie gut das Modell mit dem Parameterwert lam zu den Daten passt:
    kleiner Wert = gute Übereinstimmung, großer Wert = schlechte Übereinstimmung."""
    T_vorhergesagt = modell(t_mess, lam)
...
nicht möglich. Der verbleibende Minimalwert entspricht ungefähr der
Rauschvarianz des Sensors.

Zu Frage 2: $K(0.05)$ ist am kleinsten. Das Minimum liegt also nahe beim wahren
Wert $\lambda = 0.05\,\text{1/s}$, den wir bei der Datenerzeugung verwendet haben.

Zu Frage 3: Bei $\lambda \to 0$ kühlt das Modell kaum ab und sagt für große
Zeiten noch hohe Temperaturen voraus, während die Messwerte bereits auf
$T_\infty$ abgefallen sind. Die Abweichungen und damit $K$ werden groß.
````
...
Das ist korrekt: ein positiver Gradient bedeutet, dass $K$ nach rechts ansteigt,
das Minimum liegt also links vom aktuellen Punkt.
...
Iterationen kaum vom Startwert wegkommt. Er würde konvergieren, benötigt dafür
aber viele Millionen Schritte. Die Lernrate muss zur Skala der Kostenfunktion
passen.
````
...
Eine sinnvolle Abbruchbedingung wäre: Stopp, wenn sich $\lambda$ kaum noch
ändert, also $|\lambda_\text{neu} - \lambda_\text{alt}| < \varepsilon$ für ein
kleines $\varepsilon$. In der Praxis kombiniert man das meist mit einer
maximalen Iterationszahl als Sicherheitsgrenze.
...
for i in range(n_iterationen):
    kosten_verlauf_r.append(kosten(lam_rechts))
    grad        = ableitung_kosten(lam_rechts)
    lam_rechts  = lam_rechts - alpha * grad

fig, ax = plt.subplots()
ax.plot(kosten_verlauf,   label='Start λ = 0.01')
ax.plot(kosten_verlauf_r, label='Start λ = 0.09')
ax.set_xlabel('Iteration')
ax.set_ylabel('K(λ) in °C²')
ax.set_title('Konvergenz von zwei verschiedenen Startwerten')
ax.legend()
ax.grid(True)
plt.show()

print(f"Endwert (Start 0.01): λ = {lam_aktuell:.5f} 1/s")
print(f"Endwert (Start 0.09): λ = {lam_rechts:.5f} 1/s")
```

Zu Frage 1: Die Halbierung des Startkostenwerts ist im Plot direkt ablesbar,
je nach Lernrate in den ersten 100 bis 500 Iterationen.

Zu Frage 2: Ja. Von rechts (λ = 0.09) ist der Gradient positiv, der Schritt
also negativ, und λ nimmt ab. Beide Startwerte konvergieren zum selben Minimum.

Zu Frage 3: Am Minimum ist $K'(\lambda) \approx 0$, der Schritt also null. Der
Algorithmus verändert $\lambda$ nicht mehr und bleibt sofort stehen. Das ist
