...
for a in messwerte_liste:
    kräfte_liste.append(1.0 * a)

# Mit dem Array: eine Zeile
kräfte_array = 1.0 * messwerte_array
...
print(f"Maximale Beschleunigung: {a.max():.2f} m/s²")
print(f"Minimale Beschleunigung: {a.min():.2f} m/s²")
```
...
also Matrizen, und lösen damit lineare Gleichungssysteme wie sie in
Übungsblatt 1 auftreten.
