...
```{admonition} Mini-Übung
:class: tip
1. Was würde `b_probe == b` statt `np.allclose(b_probe, b)` liefern?
   Testen Sie es und erklären Sie das Ergebnis in einem Satz.
2. Legen Sie mit `.copy()` eine Kopie von `A` an, nennen Sie sie
   `A_falsch`, und ersetzen Sie darin `A_falsch[0, 0]` durch `4` statt `3`.
   Lösen Sie das System mit `A_falsch` und führen Sie die Probe durch.
   Was gibt die Probe aus? Was bedeutet das?
3. Warum sollte die Probe in jedem Programm stehen, auch wenn man sich
   sicher ist, die Matrix richtig eingegeben zu haben?
```
...
# Frage 1: == liefert ein Array von True/False pro Element
print(b_probe == b)        # z.B. [True True True] oder vereinzelt False
print(np.allclose(b_probe, b))  # True (robust gegenueber Rundung)

# Frage 2: falscher Eintrag
A_falsch = A.copy()        # Kopie anlegen, um A nicht zu veraendern
A_falsch[0, 0] = 4.0       # absichtlicher Fehler
x_falsch = np.linalg.solve(A_falsch, b)
print('Probe mit falschem A:', np.allclose(A_falsch @ x_falsch, b))
# True! Die Probe prueft nur, ob x zur eingegebenen Matrix passt.
# Aber: A @ x_falsch (mit dem richtigen A) schlaegt fehl:
print('Probe mit richtigem A:', np.allclose(A @ x_falsch, b))  # False
```

`==` liefert einen elementweisen Vergleich als Array, nicht ein einzelnes
True/False. Frage 2 zeigt eine wichtige Einschränkung: Die Probe prüft
nur Konsistenz, nicht Korrektheit. Ein Tippfehler in `A` erzeugt eine
Lösung, die zur falschen Matrix passt. Deshalb lohnt es sich, `A` und `b`
vor dem Lösen nochmals auszugeben und zu kontrollieren.
````
