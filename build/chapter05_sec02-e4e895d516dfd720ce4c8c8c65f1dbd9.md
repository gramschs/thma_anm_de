...
# Teilaufgabe 3: beliebiger Punkt, Längenerhalt prüfen
p = np.array([1.5, 2.3])
p_gedreht   = R @ p
p_zurueck   = R.T @ p_gedreht
laenge_vor  = np.linalg.norm(p)
laenge_nach = np.linalg.norm(p_gedreht)
print(f"\nPunkt p original: {p}")
print(f"Punkt p gedreht:  {np.round(p_gedreht, 4)}")
print(f"Punkt p zurück:   {np.round(p_zurueck, 4)}")
print(f"Gleich wie Original: {np.allclose(p, p_zurueck)}")
print(f"Länge vor Drehung:   {laenge_vor:.4f}")
print(f"Länge nach Drehung:  {laenge_nach:.4f}")
```

Zu Teilaufgabe 2: `R.T` stellt den Ausgangspunkt exakt wieder her.
Zu Teilaufgabe 3: Die Länge des Vektors bleibt durch die Drehung exakt
erhalten. Das folgt direkt aus der Orthogonalitätseigenschaft: Weil
$\mathbf{R}^\top \mathbf{R} = \mathbf{I}$ gilt, ändert $\mathbf{R}$ keine
Abstände vom Ursprung, sondern dreht den Vektor nur auf einem Kreisbogen.
Zu Teilaufgabe 4: $\mathbf{R}^\top$ ist die Inverse von $\mathbf{R}$, weil
für orthogonale Matrizen $\mathbf{R}^\top \mathbf{R} = \mathbf{I}$ gilt.
````
