...
elektrisch aufgenommene Leistung gemessen:
...
Aus diesen Messwerten erstellen wir die Abbildungen und Kennzahlen für den
Prüfstandsbericht.
````

```{admonition} Teil 1: Messdaten sichten
:class: tip
Legen Sie die drei Messreihen als Arrays an und prüfen Sie mit `.shape`, dass
alle drei gleich lang sind. Stellen Sie anschließend das Drehmoment über der
Drehzahl als Streudiagramm mit `ax.scatter()` dar, mit Achsenbeschriftung,
Titel und Gitter.
```
...
drehmoment = np.array([7.4, 6.7, 6.1, 5.4, 4.7, 4.1,
                       3.4, 2.9, 2.3, 1.7, 1.1, 0.5])
aufgenommene_leistung = np.array([1350, 1660, 1720, 1520, 1230, 980,
                                  800, 720, 660, 600, 560, 470])

print(drehzahl.shape, drehmoment.shape, aufgenommene_leistung.shape)

fig, ax = plt.subplots(figsize=(7, 4))
ax.scatter(drehzahl, drehmoment)
ax.set_xlabel('Drehzahl in 1/min')
ax.set_ylabel('Drehmoment in Nm')
ax.set_title('Gemessenes Drehmoment über der Drehzahl')
ax.grid(True)
plt.show()
```
Alle drei Arrays haben die Form `(12,)`. Wir verwenden ein Streudiagramm, weil
an zwölf einzelnen Drehzahlen gemessen wurde und zwischen den Stufen kein
Verlauf aufgenommen wurde. Das Drehmoment nimmt mit steigender Drehzahl
annähernd linear ab.
````

```{admonition} Teil 2: Leistung und Wirkungsgrad berechnen
:class: tip
Berechnen Sie zunächst die Winkelgeschwindigkeit `omega` in rad/s aus der
Drehzahl mit `omega = 2 * np.pi * drehzahl / 60`. Berechnen Sie daraus die
mechanische Leistung `p_mech = drehmoment * omega` in Watt. Berechnen Sie
schließlich den Wirkungsgrad `wirkungsgrad = p_mech / aufgenommene_leistung`.
Geben Sie alle drei Arrays aus.
```
...
print(wirkungsgrad)
```
Alle drei Rechnungen sind Vektoroperationen und wirken auf das ganze Array. Der
Wirkungsgrad liegt zwischen rund 0.14 und 0.84 und ist damit überall kleiner
als 1, wie es für einen realen Motor sein muss. Er steigt zunächst mit der
Drehzahl an, erreicht ein Maximum und fällt danach wieder ab.
````

```{admonition} Teil 3: Kennlinienfeld als Subplots
:class: tip
...
plt.show()
```
Für den Bericht behandeln wir die Messpunkte als Kennlinie und verbinden sie
mit einer Linie, um den Verlauf zu zeigen. Das Kennlinienfeld zeigt auf einen
Blick: Das Drehmoment fällt linear, die mechanische Leistung hat ihr Maximum in
der Mitte des Drehzahlbereichs, und der Wirkungsgrad wird erst bei hoher
Drehzahl gut.
````

```{admonition} Teil 4: Betriebspunkte bestimmen
:class: tip
...
````{admonition} Teil 5: Wirkungsgrad mit Messunsicherheit
:class: tip
