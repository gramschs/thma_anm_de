...
Zerlegung der Verschiebung $\vec{u}_j$ an Knoten $j$ in zwei Anteile: Die
Komponente $\delta\,\vec{e}$ entlang der Stabachse (orange) dehnt oder staucht
den Stab und erzeugt die Normalkraft $N = k \cdot \delta$. Die senkrechte
Komponente (grau gestrichelt) bewegt den Knoten quer zur Stabachse und leistet
keinen Beitrag zur Stabkraft. Der Einheitsvektor $\vec{e}$ zeigt in Richtung
des Stabs unter dem Winkel $\varphi$ gegenüber der $x$-Achse. (Quelle: eigene
