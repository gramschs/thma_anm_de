...
| RK45 in `solve_ivp` | $O(\Delta t^5)$ (Lösung) | adaptiv, automatisch | Standardfall in der Praxis |

Halbiert man die Schrittweite, sinkt der Fehler bei Euler auf die Hälfte, bei
RK45 wesentlich schneller (etwa mit der Größenordnung $\Delta t^5$, wobei in der
Praxis die adaptive Schrittweitensteuerung noch eine Rolle spielt.
```
