...
Die Zeile setzt mehrere Bausteine aus diesem Abschnitt zusammen: `np.exp(-delta * t)`
beschreibt die abklingende Hüllkurve, `np.sin(2 * np.pi * f * t)` die
eigentliche Schwingung, und die Multiplikation der beiden Arrays kombiniert sie
elementweise zu einem einzigen Signal. Alles ohne Schleife, in einer Zeile Code.
Dieses Signal `a` verwenden wir im nächsten Abschnitt, um statistische
Kenngrößen zu berechnen.
