...
ausgelenkt werden. Das Produkt $\mathvec{F} = \mathbf{K}\cdot\vec{x}$ ist die
...
Der Verschiebungsvektor der beiden Massen beträgt $\vec{x} = \begin{pmatrx} 0.5
& 0.3\end{pmatrix} in mm.

1. Berechnen Sie den Kraftvektor $\mathvec{F} = \mathbf{K}\cdot\vec{x}$.
