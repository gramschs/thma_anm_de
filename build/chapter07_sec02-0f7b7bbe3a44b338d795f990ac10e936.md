...
# Vergleich: tatsächlicher Fehler vs. Taylor-Vorhersage (Vorwärtsdifferenz)
print("Vorwärtsdifferenz:")
print(f"{'dx':>8}  {'Fehler':>10}  {'Theorie (dx/2)*|f´´|':>22}")
print("-" * 46)
for dx in [0.1, 0.01, 0.001]:
    fehler_num  = abs(exakt - vorwaerts(f, x0, dx))
    fehler_theo = (dx / 2) * abs(f_zweimal(x0))
    print(f"{dx:>8.3f}  {fehler_num:>10.2e}  {fehler_theo:>22.2e}")

print()

# Vergleich: tatsächlicher Fehler vs. Taylor-Vorhersage (zentraler Differenzenquotient)
print("Zentraler Differenzenquotient:")
print(f"{'dx':>8}  {'Fehler':>10}  {'Theorie (dx²/6)*|f´´´|':>24}")
print("-" * 48)
for dx in [0.1, 0.01, 0.001]:
    fehler_num  = abs(exakt - zentral(f, x0, dx))
    fehler_theo = (dx**2 / 6) * abs(f_dreimal(x0))
    print(f"{dx:>8.3f}  {fehler_num:>10.2e}  {fehler_theo:>24.2e}")
```
...
# Maschinengenauigkeit: kleinste Zahl eps, für die 1.0 + eps != 1.0 gilt
...
def g_strich(x):
    """Ableitung von exp(x) ist wieder exp(x)."""
    return np.exp(x)
...
# --- Frage 1 und 2: alle drei Formeln für sin(x) ---
x0    = 1.0
exakt = f_strich(x0)
...
# optimale Schrittweiten aus der Formel
eps = np.finfo(float).eps
print(f"Optimales dx (Vorwärtsdiff.):       sqrt(eps) = {np.sqrt(eps):.2e}")
print(f"Optimales dx (Zentraler Diff.): eps ** (1/3) = {eps**(1/3):.2e}")

# --- Frage 3: exp(x) bei x0 = 0 ---
x1      = 0.0
exakt_g = g_strich(x1)
