...
:class: caution
Es gibt zwei gängige Konventionen, wie man Messdaten in einer Matrix anordnet 
und beide sind in der Praxis verbreitet.

In diesem Kapitel wählen wir: **eine Zeile pro Sensor, eine Spalte pro
Zeitpunkt**. Der Vorteil: Der vollständige Signalverlauf eines Sensors liegt
als zusammenhängende Zeile vor und lässt sich mit `messungen[i, :]` direkt
abrufen. Diese Anordnung passt außerdem gut zur Matrix-Vektor-Multiplikation,
die wir im nächsten Abschnitt kennenlernen: Eine Kalibrierungsmatrix `K` wirkt
dann mit `K @ messungen` auf jeden Sensor als Ganzes.

Die **umgekehrte Konvention** — eine Zeile pro Zeitpunkt, eine Spalte pro
Sensor — ist ebenfalls verbreitet, insbesondere in der Datenanalyse und im
maschinellen Lernen. Dort entspricht jede Zeile einer Beobachtung und jede
Spalte einem Merkmal (Feature), was dem sogenannten *Tidy-Data*-Format
entspricht. Bibliotheken wie pandas oder scikit-learn erwarten Daten meist in
dieser Form.

Welche Konvention die richtige ist, hängt davon ab, welche Operation als
Nächstes folgt. Entscheidend ist, die gewählte Konvention konsistent zu
verwenden und sie klar zu dokumentieren.
```
