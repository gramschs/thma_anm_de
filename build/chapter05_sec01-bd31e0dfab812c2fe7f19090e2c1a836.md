...
# Rückdrehung von C: zuerst drehen, dann mit Transponierter zurückdrehen
punkt_C = np.array([1.0, 2.0])
punkt_C_gedreht = R @ punkt_C
punkt_C_zurueck = R.T @ punkt_C_gedreht

print(f"C original:  {punkt_C}")
print(f"C gedreht:   {np.round(punkt_C_gedreht, 4)}")
print(f"C zurück:    {np.round(punkt_C_zurueck, 4)}")
```

Zu Frage 1: $\mathbf{R}^\top \mathbf{R} = \begin{pmatrix} 0 & 1 \\ -1 & 0
\end{pmatrix} \begin{pmatrix} 0 & -1 \\ 1 & 0 \end{pmatrix} = \begin{pmatrix}
1 & 0 \\ 0 & 1 \end{pmatrix} = \mathbf{I}$. Zu Frage 3: Per Definition einer
orthogonalen Matrix gilt $\mathbf{R}^\top \mathbf{R} = \mathbf{I}$, also ist
die Transponierte bereits die Inverse.
````
...
$\cos 30° = \frac{\sqrt{3}}{2} \approx 0.866$, $\sin 30° = \frac{1}{2}$,
$\cos 20° \approx 0.940$, $\sin 20° \approx 0.342$,
$\cos 10° \approx 0.985$, $\sin 10° \approx 0.174$ ergibt sich:
...
**Schritt 2: Nicken um $\beta = 20°$ (Drehung in der $xz$-Ebene)**

\begin{equation*}
\vec{p}^{(2)} = \mathbf{R}_y(20°)\,\vec{p}^{(1)}
= \begin{pmatrix}
    \cos 20° & 0 & \sin 20° \\
    0        & 1 & 0        \\
   -\sin 20° & 0 & \cos 20°
  ...
  \begin{pmatrix} 1.732 \\ 1.000 \\ 0 \end{pmatrix}
= \begin{pmatrix}
    0.940 \cdot 1.732 + 0.342 \cdot 0 \\
    1.000 \\
   -0.342 \cdot 1.732 + 0.940 \cdot 0
  \end{pmatrix}
= \begin{pmatrix} 1.628 \\ 1.000 \\ -0.593 \end{pmatrix}.
\end{equation*}

**Schritt 3: Rollen um $\gamma = 10°$ (Drehung in der $yz$-Ebene)**

\begin{equation*}
\vec{p}^{(3)} = \mathbf{R}_x(10°)\,\vec{p}^{(2)}
= \begin{pmatrix}
    1 & 0         &  0        \\
    0 & \cos 10°  & -\sin 10° \\
    0 & \sin 10°  &  \cos 10°
...
= \begin{pmatrix}
    1.628 \\
    0.985 \cdot 1.000 + (-0.174) \cdot (-0.593) \\
    0.174 \cdot 1.000 + 0.985 \cdot (-0.593)
  \end{pmatrix}
= \begin{pmatrix} 1.628 \\ 1.088 \\ -0.410 \end{pmatrix}.
\end{equation*}

Das Ergebnis stimmt mit der Python-Ausgabe oben überein. Man sieht, dass
nach dem reinen Gieren (Schritt 1) die $z$-Komponente noch null ist; erst
das Nicken (Schritt 2) hebt Punkt B aus der $xy$-Ebene heraus.
