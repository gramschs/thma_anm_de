...
Wir beginnen direkt mit dem Code. Das Fachwerk hat drei Knoten: zwei
Lagerknoten und einen freien Knoten in der Mitte, an dem die Last angreift.
...
Die **Konnektivitätsmatrix** `verbindung` ist das Herzstück der Fachwerk-
beschreibung. Eine 1 an Position `[i, j]` bedeutet: zwischen Knoten $i$
und Knoten $j$ existiert ein Stab. Die Matrix ist symmetrisch, weil ein Stab
von $i$ nach $j$ dasselbe ist wie von $j$ nach $i$.
