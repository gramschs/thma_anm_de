...
:class: tip
1. Definieren Sie die Transformation für Segment 1: Drehung um den
   Schulterwinkel $\alpha = 30°$ im Ursprung, dann Verschiebung um
   $(l_1, 0)^\top = (3, 0)^\top$ entlang der lokalen $x$-Achse.
   Berechnen Sie die Position des Ellenbogengelenks (Ende von Segment 1).
2. Definieren Sie die Transformation für Segment 2: Drehung um den
   Ellenbogenwinkel $\beta = -45°$ relativ zu Segment 1, dann
   Verschiebung um $(l_2, 0)^\top = (2, 0)^\top$. Verketten Sie beide
   Transformationen und berechnen Sie die Greiferspitze.
3. Zeichnen Sie den Greifarm: Ursprung, Ellenbogengelenk und
   Greiferspitze als Punkte, verbunden durch Linien. Beschriften Sie
   die drei Punkte.
4. Variieren Sie $\alpha$ von $0°$ bis $180°$ in 13 Schritten und
   behalten Sie $\beta = -45°$ fest. Zeichnen Sie für jeden Schritt
   die Greiferspitze als kleinen Punkt. Was ist die geometrische Figur,
   die die Greiferspitzen beschreiben?
```
...
# Teilaufgaben 1 und 2: Transformationen aufbauen und verketten
# T1: Schulter dreht den Arm, dann Verschiebung entlang lokaler x-Achse
T1 = transformationsmatrix(alpha, l1, 0.0)
# T2: Ellenbogen dreht relativ zu Segment 1, dann Verschiebung
T2 = transformationsmatrix(beta,  l2, 0.0)

# Ursprung, Ellenbogen, Greiferspitze in homogenen Koordinaten
ursprung    = np.array([0.0, 0.0, 1.0])
ellenbogen  = T1 @ ursprung
greifer     = T1 @ T2 @ ursprung    # Gesamttransformation: erst T2, dann T1
...
# Teilaufgabe 4: Trajektorie der Greiferspitze
alphas = np.linspace(0, np.pi, 13)
for a in alphas:
    T1_var = transformationsmatrix(a, l1, 0.0)
    g = T1_var @ T2 @ ursprung
...
Die Greiferspitzen beschreiben einen Kreisbogen. Das liegt daran, dass
$\beta$ und damit der relative Winkel zwischen den Segmenten fest bleibt.
Die Greiferspitze hat einen konstanten Abstand vom Ursprung, der durch
die Längen $l_1$, $l_2$ und den festen Winkel $\beta$ bestimmt wird.
Würde man auch $\beta$ variieren, könnte die Greiferspitze jeden Punkt
innerhalb eines Kreisrings erreichen.
````
