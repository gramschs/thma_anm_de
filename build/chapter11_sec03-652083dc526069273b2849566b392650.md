...
Dazu kommt die Straße selbst: Fahrbahnrillen regen das Fahrzeug periodisch
an. Trifft die Anregungsfrequenz die Eigenfrequenz des Fahrwerks, wächst
die Amplitude stark an. Unser Modell erweitert sich zu
...
for D, farbe in zip(D_werte, farben):
    ...
    # Phasenporträt: bei D < 1 Spirale, bei D >= 1 monotoner Abfall
...
Für $D = 1$ und $D = 2$ gibt es kein Vorzeichenwechsel der Auslenkung, der
Arm kriecht monoton in den Ursprung. Die gedämpfte Eigenfrequenz beträgt
...
Zu Frage 2: Die Kurve im Phasenporträt hat keinen Vorzeichenwechsel auf der
$x$-Achse. Sie verläuft vollständig auf einer Seite und wandert monoton in
den Ursprung, statt ihn zu umkreisen.
...
eine periodische Anregung dazu: $F(t) = F_0\sin(\Omega t)$. Das könnte
eine Fahrbahnrille sein, die das Fahrwerk mit der Frequenz $\Omega$ anregt.
...
# --- Simulation bei Resonanz: Omega = omega_0 ---
Omega_res = omega_0
t_end     = 30.0
...
# Analytische Formel zur Verifikation: A = F0 / (k * 2*D*sqrt(1-D^2))
A_stat_ana = F0 / (k_feder * 2 * D * np.sqrt(1 - D**2))
...
ax.set_title(f'Erzwungene Schwingung bei Resonanz (Ω = ω₀ = {omega_0:.1f} rad/s, D = {D})')
...
Bei $\Omega = \omega_0$ ist die Energieübertragung von der Anregung auf den
Schwinger am größten. Das nennt man **Resonanz**. Für kleine Dämpfung ($D
\ll 1$) kann die Resonanzamplitude ein Vielfaches der statischen Auslenkung
$F_0/k$ betragen.
...
Zu Frage 3: Für $D \to 0$ strebt der Nenner $2D\sqrt{1-D^2} \to 0$, die
stationäre Amplitude $A = F_0/(k \cdot 2D\sqrt{1-D^2}) \to \infty$.
...
ohne Überschwingen zurück, für $D > 1$ kriecht es monoton in den
...
als `max(|x|)` ablesen. Resonanz tritt auf, wenn $\Omega \approx \omega_0$:
dann ist die Amplitude maximal.
