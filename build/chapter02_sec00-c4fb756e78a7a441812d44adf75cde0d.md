...
1. Berechnen Sie die Gesamtkraft `gesamtkraft_kn`, die an jedem Zeitpunkt
   auf den Kran wirkt (Summe beider Seile).
2. Rechnen Sie `gesamtkraft_kn` in Newton um und speichern Sie das Ergebnis
   in `gesamtkraft_newton` (1 kN = 1000 N).
...
gesamtkraft_kn = seil_1 + seil_2
print(gesamtkraft_kn)

gesamtkraft_newton = gesamtkraft_kn * 1000.0
print(gesamtkraft_newton)
