...
Nun übertragen wir das in Python-Code. Der Dreischritt lässt sich dabei
direkt in der Formel ablesen:

1. **Projizieren**: Die Relativverschiebung $\vec{u}_j - \vec{u}_i$ wird
   auf die Stabachse projiziert. Der parallele Anteil ist
   \begin{equation*}
...
3. **Zurückverteilen**: Das Produkt $\vec{e}\,\vec{e}^\top$ ist genau die
   Elementsteifigkeitsmatrix. Sie projiziert zunächst auf die Stabachse
   und verteilt die Kraft dann wieder auf $x$- und $y$-Richtung:
   \begin{equation*}
   \mathbf{k}_{ij}
   = k\,\vec{e}\vec{e}^\top
   = k
...
Im Code entspricht `cos_w` und `sin_w` den Komponenten von $\vec{e}$;
