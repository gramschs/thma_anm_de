...
```python
# Frage 1:
# Mit Knoten in den Spalten liefert knoten_pos[0, :] direkt alle
# x-Koordinaten und knoten_pos[1, :] alle y-Koordinaten als Vektoren.
# Das ist praktisch beim Plotten: ax.scatter(knoten_pos[0, :], knoten_pos[1, :])
# funktioniert ohne Umformung.

# Frage 2:
print(knoten_pos.shape)   # (2, 3)
# shape[0] = 2: Anzahl der Zeilen = Anzahl der Koordinatenrichtungen (x und y)
# shape[1] = 3: Anzahl der Spalten = Anzahl der Knoten
# shape[0] ist für ein ebenes Fachwerk immer 2, sagt also nichts über
# die Größe des Fachwerks aus.
...
`.shape[0]` gibt immer 2 zurück (zwei Koordinatenrichtungen im ebenen Problem)
und ist daher keine sinnvolle Knotenanzahl.
````
...
```{admonition} Mini-Übung
:class: tip
Die zweite Code-Zelle mit `verbindung` und `kraft_vektor` wurde bereits ausgeführt.


1. Betrachten Sie die Konnektivitätsmatrix `verbindung`.  

   Warum hat die Diagonale ausschließlich Nullen?  
   Begründen Sie in einem Satz, ohne Code auszuführen.


2. `kraft_vektor` hat 6 Einträge für 3 Knoten.  

   An welchem Index würde eine horizontale Kraft an Knoten 2 stehen?  
   Berechnen Sie den Index im Kopf und überprüfen Sie dann mit Code.
```

```{code-cell} python

### Optional: Antworten überprüfen

print("Konnektivitätsmatrix:")
print(verbindung.astype(int))

print("\nKraftvektor (Index : Wert):")
for i, f in enumerate(kraft_vektor):
    print(f"  Index {i}: {f:.0f} N")
```
...
```python
# Frage 1:
# Die Diagonale enthält verbindung[i, i], also die Verbindung eines Knotens
# mit sich selbst. Ein Stab von i nach i existiert nicht, daher sind
# alle Diagonaleinträge null.

# Frage 2:
# Horizontale Kraft (x-Richtung) an Knoten n liegt bei Index 2*n.
# Für Knoten 2: Index 2*2 = 4.
print("Index für Fx an Knoten 2:", 2 * 2)   # 4
print("Aktueller Wert:", kraft_vektor[4])    # 0.0 N (keine Kraft angelegt)
```

Eine horizontale Kraft an Knoten 2 gehört an Index 4, weil die Einträge
paarweise angeordnet sind: `[Fx_0, Fy_0, Fx_1, Fy_1, Fx_2, Fy_2]`.
Die Diagonale von `verbindung` ist null, weil kein Knoten einen Stab zu
