...
# 4.3 Scatter-Plots, Histogramme und Fehlerbalken
...
aus Kapitel 3 untersuchen wollen, brauchen wir ein ganz anderes Diagramm:
...
* [ ] Sie können Scatter-Plot und Linienplot in einem gemeinsamen Diagramm
  kombinieren.
* [ ] Sie können mit `ax.imshow()` NumPy-Arrays als Bilder darstellen.
```
...
ax.scatter(temperatur, beschleunigung, color='steelblue', marker='o')
...
2. Verwenden Sie rote Dreiecke als Marker (`marker='^'`, `color='tomato'`).
...
fig, ax = plt.subplots(figsize=(7, 5))
ax.scatter(daempf_werte, max_beschl, color='tomato', marker='^', s=60)
...
Wir analysieren das Messrauschen unseres Sensors. In Kapitel 3 haben wir
...
ax.hist(rauschen, bins=30, color='steelblue', edgecolor='white')
...
ax.plot(x_kurve, norm.pdf(x_kurve, 0, 0.5),
        color='tomato', linewidth=2, label='Normalverteilung (σ=0.5)')
...
Histogramm mit der theoretischen Dichtefunktion `norm.pdf()` vergleichen.
Die gute Übereinstimmung bestätigt unsere Rauschmodellierung aus Kapitel 3.
...
ax[0].hist(rauschen_normal, bins=40, color='steelblue',
...
ax[1].hist(rauschen_gleich, bins=40, color='seagreen',
           edgecolor='white', density=True)
...
Störquellen entsteht. Das entspricht Übung 5 aus Übungsblatt 0.
````
...
Das entspricht direkt Übung 3 auf Übungsblatt 0:

```{code-cell} python
# Lineare Regression (Formeln aus Uebung 3)
N      = len(temp_punkte)
...
ax.errorbar(temp_punkte, beschl_mittel, yerr=beschl_std,
            ...
            label='Messwerte ± Std')
ax.plot(x_linie, y_linie, color='tomato', linewidth=1.5,
...
Die Kombination aus Scatter-Plot (oder hier `errorbar`) und Linienplot in
einem gemeinsamen Diagramm ist ein sehr häufiges Muster in der
Ingenieurpraxis: Messwerte als Punkte, Modell oder Regression als Linie.

## Bilder anzeigen mit `ax.imshow()`

NumPy-Arrays lassen sich nicht nur als Diagramme darstellen, sondern auch
als Bilder. Ein Graustufenbild ist nichts anderes als eine zweidimensionale
Matrix, bei der jeder Eintrag die Helligkeit eines Pixels beschreibt. Ein
Farbbild ist eine dreidimensionale Matrix der Form `(Hoehe, Breite, 3)`,
wobei die drei Schichten den Rot-, Grün- und Blaukanal enthalten.

`ax.imshow()` stellt ein solches Array als Bild dar. Das ist direkt relevant
für Übung 6 auf Übungsblatt 0, wo ein Bild eingelesen, als Array gespeichert
und dann rotiert wird.

Wir erzeugen zunächst ein synthetisches Graustufenbild, um das Prinzip zu
verstehen:
...
# Synthetisches Graustufenbild: 100x100 Pixel
np.random.seed(0)
bild = np.zeros((100, 100))
for i in range(100):
    for j in range(100):
        bild[i, j] = np.sin(i / 10) * np.cos(j / 10)

fig, ax = plt.subplots(figsize=(5, 5))
im = ax.imshow(bild, cmap='gray')
fig.colorbar(im, ax=ax, label='Intensitaet')
ax.set_title('Synthetisches Graustufenbild')
ax.set_xlabel('Spalte (Pixel)')
ax.set_ylabel('Zeile (Pixel)')
plt.show()
```

`cmap='gray'` wählt eine Graustufenfarbskala. Ohne `cmap`-Argument verwendet
Matplotlib eine Farbskala, was für Graustufenbilder ungeeignet ist. Die
Colorbar zeigt die Abbildung von Zahlenwerten auf Grautöne.

Für echte Bilddaten verwenden wir die `PIL`-Bibliothek zum Einlesen und
konvertieren das Bild dann in ein NumPy-Array. Genau das verlangt Übung 6:

```{code-cell} python
from PIL import Image

# Bild einlesen und als NumPy-Array speichern
# bild_original = np.array(Image.open('mein_bild.jpg'))

# Zum Demonstrieren erzeugen wir ein synthetisches RGB-Bild (50x80 Pixel)
hoehe, breite = 50, 80
bild_rgb = np.zeros((hoehe, breite, 3), dtype=np.uint8)
bild_rgb[:, :, 0] = np.linspace(0, 255, breite)           # Rotkanal
bild_rgb[:, :, 1] = np.linspace(0, 255, hoehe).reshape(-1, 1)  # Gruenkanal

# Rotation um 90° durch Transposition und Zeilenumkehrung
bild_transponiert = bild_rgb.transpose(1, 0, 2)   # Achsen tauschen
bild_rotiert      = bild_transponiert[:, ::-1, :]  # Zeilen umkehren

fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(10, 4))

ax[0].imshow(bild_rgb)
ax[0].set_title('Originalbild')
ax[0].set_xlabel('Breite in Pixel')
ax[0].set_ylabel('Hoehe in Pixel')

ax[1].imshow(bild_rotiert)
ax[1].set_title('Rotiert um 90°')
ax[1].set_xlabel('Breite in Pixel')
ax[1].set_ylabel('Hoehe in Pixel')

plt.tight_layout()
plt.show()
```

`transpose(1, 0, 2)` tauscht die ersten beiden Achsen (Zeilen und Spalten),
lässt aber die dritte Achse (Farbkanäle) unverändert. `[:, ::-1, :]` kehrt
die Reihenfolge der Spalten um. Die Kombination beider Operationen entspricht
einer Rotation um 90° im Uhrzeigersinn, wie in Übung 6 beschrieben.

```{admonition} Bildkoordinaten
:class: note
Bei `ax.imshow()` verläuft die y-Achse standardmäßig **von oben nach unten**,
also entgegengesetzt zur mathematischen Konvention. Das ist die übliche
Bildkonvention: Zeile 0 ist oben links. Für die meisten Bildverarbeitungs-
aufgaben ist das korrekt. Wer die Achsenrichtung umkehren möchte, kann
`ax.invert_yaxis()` verwenden.
```
