...
Das Wärmeübertragungsgesetz (analog zum Ohmschen Gesetz) lautet für jede
Schicht:
...
Das Ergebnis lässt sich direkt überprüfen: Da $T_{CR} > T_{LA}$, fließt Wärme
von rechts nach links. Der Wärmestrom $Q$ ist negativ (die Vorzeichenkonvention
in unserem Modell definiert positives $Q$ von links nach rechts).
...
## Ausblick: Parameterstudien

In der Ingenieurpraxis fragt man oft: Was passiert, wenn ich die Dämmstärke
verändere? Wie hängt der Wärmestrom $Q$ vom Widerstand $R_B$ ab? Das
Werkzeug dafür — eine Schleife über viele Parameterwerte mit anschließender
grafischer Auswertung — erarbeiten wir in Abschnitt 3.6 am Beispiel der
Messbrücke. Das dort eingeübte Muster lässt sich direkt auf dieses
Wärmedurchgangsproblem übertragen.
