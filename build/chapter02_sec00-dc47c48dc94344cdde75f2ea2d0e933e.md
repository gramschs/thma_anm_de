...
`.shape` gibt die Abmessungen des Arrays als Tupel zurück. `(5,)` bedeutet:
eine Dimension mit fünf Elementen. `.dtype` gibt den gemeinsamen Datentyp
aller Elemente zurück, hier `float64` für Fließkommazahlen. Beide Attribute
werden uns durch das gesamte Kapitel begleiten, sie sind der schnellste Weg,
ein unbekanntes Array zu prüfen.

### Arrays erzeugen

Neben `np.array()`, das eine bestehende Werteliste in ein Array umwandelt,
stellt NumPy zwei weitere Funktionen bereit, mit denen wir Arrays direkt
erzeugen, ohne die Werte einzeln aufzuschreiben.

`np.linspace(start, stop, anzahl)` erzeugt `anzahl` gleichmäßig verteilte
Werte zwischen `start` und `stop`. Das eignet sich hervorragend für
Zeitachsen:
...
```{admonition} Wie viele Nullen?
:class: note
Das Argument von `np.zeros()` legt die Länge des Arrays fest, nicht den
Wert der Elemente. `np.zeros(5)` erzeugt also fünf Nullen, keine Zahl mit
dem Wert 5.
```
