...
# np.column_stack haengt b als neue Spalte rechts an A an
...
:class: note
Für quadratische Matrizen sind beide Kriterien gleichwertig:
$\det(\mathbf{A}) \neq 0$ genau dann, wenn $\text{rang}(\mathbf{A}) = n$.
Der Rang ist aber allgemeiner, weil er auch für nicht-quadratische Matrizen
definiert ist. Für die meisten Ingenieuraufgaben reicht der Determinanten-Test.
```
...
A = np.array([[1, 2], [2, 4]], dtype=float)  # gleich fuer beide Systeme
b1 = np.array([5., 10.])
b2 = np.array([5., 11.])

# Frage 1: Determinante (gilt fuer beide, da A gleich)
...
for name, b in [('System 1', b1), ('System 2', b2)]:
    ...
    print(f'{name}: rang(A)={rA}, rang([A|b])={rAb}', end='  ->  ')
    if rA == rAb:
        print('unendlich viele Lösungen')
    else:
        print('keine Lösung')
