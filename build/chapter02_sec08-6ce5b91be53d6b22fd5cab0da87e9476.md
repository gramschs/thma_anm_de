...
- Links: Scatter-Plot mit Temperatur auf der x-Achse und Gasverbrauch auf der
  y-Achse, überlagert mit der Regressionsgerade. Markieren Sie den
  Vorhersagepunkt aus Teil 3 auffällig.
- Rechts: Zwei Subplots untereinander — oben der monatliche Temperaturverlauf,
  unten der monatliche Gasverbrauch, jeweils als Linienplot mit Markern und
  den Monatsnamen auf der x-Achse.
...
ax[0, 0].scatter(temperatur, gasverbrauch,
                 color='#4C72B0', s=70, zorder=5, label='Monatsdaten')
ax[0, 0].plot(x_linie, y_linie, color='#DD8452', linewidth=1.5,
              label=f'Regression: y = {m:.1f}x + {b:.0f}')
ax[0, 0].scatter(-5, vorhersage, marker='*', s=250, color='#C44E52',
                 zorder=6, label=f'Vorhersage −5 °C: {vorhersage:.0f} kWh')
ax[0, 0].set_xlabel('Außentemperatur in °C')
ax[0, 0].set_ylabel('Gasverbrauch in kWh')
ax[0, 0].set_title('Regression: Temperatur vs. Gasverbrauch')
...
# Oben rechts: monatliche Temperatur
ax[0, 1].plot(np.arange(12), temperatur, color='#4C72B0',
              marker='o', markersize=5, linewidth=1.5)
ax[0, 1].set_ylabel('Außentemperatur in °C')
ax[0, 1].set_title('Monatliche Außentemperatur')
ax[0, 1].set_xticks(np.arange(12))
ax[0, 1].set_xticklabels(monate, rotation=45)
ax[0, 1].grid(True)

# Unten rechts: monatlicher Gasverbrauch
ax[1, 1].plot(np.arange(12), gasverbrauch, color='#DD8452',
              marker='o', markersize=5, linewidth=1.5)
ax[1, 1].set_ylabel('Gasverbrauch in kWh')
ax[1, 1].set_title('Monatlicher Gasverbrauch')
