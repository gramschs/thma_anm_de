...
```{code-cell} python
print(A[0, 2])   # Zeile 0, Spalte 2: Clementinen an Tag 1
print(A[1, :])   # Zeile 1 komplett:  Einkauf an Tag 2
print(A[:, 0])   # Spalte 0 komplett: Äpfel an allen Tagen
```
...
3. Wie viele Clementinen wurden insgesamt über alle drei Tage eingekauft?
...
print(A[:, 2])          # [1. 0. 3.] -> Clementinen an Tag 1, 2, 3
print(np.sum(A[:, 2]))  # 4.0 -> insgesamt 4 Clementinen
```

`A[2, 1]` ist der Eintrag in Zeile 2 (Tag 3), Spalte 1 (Bananen): an Tag 3 wurde
eine Banane gekauft. `A[:, 2]` ist die gesamte Spalte 2 (Clementinen): 1, 0 und
3 Clementinen an den drei Tagen. `np.sum` addiert alle Einträge dieser Spalte.
````
...
1. In welcher Zeile von `A` steckt die Information über Tag 2? Welchen
   Eintrag müssten Sie ändern, wenn an Tag 2 doch eine Clementine gekauft
   worden wäre?
...
# Frage 1: Tag 2 steckt in Zeile 1 (Zählung ab 0)
# eine Clementine mehr: A[1, 2] ändern
A[1, 2] = 1.0

# Frage 2: A[1,2] gibt 1.0 zurück
...
Koeffizient der dritten Unbekannten (Clementinen) in der zweiten Gleichung.
...
definiert ist. Für kleine Systeme und zum theoretischen Verständnis reicht der Determinanten-Test.
```
