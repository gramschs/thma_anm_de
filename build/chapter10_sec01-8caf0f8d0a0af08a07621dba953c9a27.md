...
zu einem festen Zeitpunkt, z.B. bei $t = 20~\text{min}$. Der Fehler, den ein
einzelner Euler-Schritt macht, heißt **lokaler Fehler**; der Abstand zwischen
numerischer und analytischer Lösung nach vielen Schritten heißt **globaler
Fehler**. Der im nächsten Plot gezeigte Fehler bei $t = 20~\text{min}$ ist also
ein globaler Fehler.
...
Der Fehler, den ein einzelner Euler-Schritt macht, heißt **lokaler Fehler**; der
Abstand zwischen numerischer und analytischer Lösung nach vielen Schritten heißt
**globaler Fehler**. Der im Plot gezeigte Fehler bei $t = 20~\text{min}$ ist
also ein globaler Fehler.

Der Plot des Fehlers zeigt eine typische **Konvergenzgerade** im doppelt
logarithmischen Maßstab: Halbieren wir die Schrittweite, halbiert sich auch der
Fehler ungefähr. Das bedeutet eine **Konvergenzordnung** von $O(\Delta t)$. Das
Euler-Verfahren wird deshalb als ein Verfahren erster Ordnung bezeichnet. Zum
Vergleich: Die Trapezregel aus Kapitel 9 hatte Ordnung $O(\Delta t^2)$. In
Abschnitt 10.3 lernen wir Verfahren kennen, die Ordnung 4 oder höher erreichen.
