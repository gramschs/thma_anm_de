...
````{admonition} Mini-Übung
:class: tip
Prüfen Sie für die folgenden vier Systeme jeweils mit Determinante
**und** Rang, ob eine eindeutige Lösung existiert. System 1 kennen Sie
bereits aus der Café-Aufgabe — versuchen Sie, das Ergebnis durch
Inspektion der Zeilen vorherzusagen.

**System 1 (Café):**
$$\mathbf{A_1} = \begin{pmatrix} 5 & 2 & 3 \\ 3 & 4 & 1 \\ 6 & 1 & 4 \end{pmatrix}, \quad
\vec{b_1} = \begin{pmatrix} 24.50 \\ 17.30 \\ 28.10 \end{pmatrix}$$

**System 2:**
$$\mathbf{A_2} = \begin{pmatrix} 1 & 2 \\ 3 & 4 \end{pmatrix}, \quad
\vec{b_2} = \begin{pmatrix} 5 \\ 6 \end{pmatrix}$$

**System 3:**
$$\mathbf{A_3} = \begin{pmatrix} 1 & 2 \\ 2 & 4 \end{pmatrix}, \quad
\vec{b_3} = \begin{pmatrix} 5 \\ 10 \end{pmatrix}$$

**System 4:**
$$\mathbf{A_4} = \begin{pmatrix} 1 & 2 \\ 2 & 4 \end{pmatrix}, \quad
\vec{b_4} = \begin{pmatrix} 5 \\ 11 \end{pmatrix}$$
````
...
systeme = [
    (np.array([[5,2,3],[3,4,1],[6,1,4]], dtype=float), np.array([24.50, 17.30, 28.10])),
...
Ausgabe:
```
System 1: det=-13.0, rang(A)=3, rang([A|b])=3 → Genau eine Lösung
System 2: det=-2.0,  rang(A)=2, rang([A|b])=2 → Genau eine Lösung
System 3: det=0.0,   rang(A)=1, rang([A|b])=1 → Unendlich viele Lösungen
System 4: det=0.0,   rang(A)=1, rang([A|b])=2 → Keine Lösung
```

System 1 (Café): Die Determinante ist ungleich null — das System hat eine
eindeutige Lösung, die Einzelpreise lassen sich bestimmen. System 3: Die
zweite Gleichung ($2x + 4y = 10$) ist das Doppelte der ersten — keine neue
Information, unendlich viele Lösungen. System 4: Die zweite Gleichung
widerspricht der ersten ($2 \cdot 5 = 10 \neq 11$) — kein gemeinsamer
Schnittpunkt.
````
