...
Feder sitzt. Das Newtonsche Gesetz $ma = F$ ergibt für kleine Auslenkungen um
die Gleichgewichtslage für diese freie Schwingung
...
in zwei skalare Diffenretialgleichungen 1. Ordnung umschreiben und damit
`solve_ivp` ohne einsetzen können.
...
* [ ] Sie können eine skalare **DGL 2. Ordnung** durch die Substitution $y_1 =
  x$, $y_2 = \dot{x}$ in ein System zweier Differentialglechungen 1. Ordnung
...
axes[0].plot(t_eval, sol.y[0] * 100, label='solve_ivp (RK45)')
axes[0].plot(t_ref, x_ref * 100, linestyle='--', linewidth=1.5, label='analytisch')
...
System Energie. Im Schwingfall ($D < 1$) werden die Ellipsen zu nach innen
spiralierenden Kurven.
