...
Die Idee ist einfach: Wir benennen die beiden Größen, die wir berechnen wollen,
...
\begin{equation*}
\dot{y}_1 = y_2, \qquad \dot{y}_2 = -\left(\frac{k}{m}\right)y_1.
\end{equation*}

Diese schreiben wir in Vektornotation:

$$\frac{d}{dt}\begin{pmatrix} y_1 \\ y_2 \end{pmatrix}
= \begin{pmatrix} y_2 \\ -\dfrac{k}{m}\,y_1 \end{pmatrix}.$$

In Kapitel 10 hatte `y` einen Eintrag ($y = [T]$), hier hat `y` zwei Einträge:
$y = [x,\, \dot{x}]$. Das ist der einzige Unterschied im Code. Genau dieses
Vorgehen klappt für jede skalare DGL zweiter Ordnung: Wir wählen als Zustände
die gesuchte Größe und ihre erste Ableitung.
...
wie schnell die Feder die Masse zurückzieht. Die Karosserie schwingt etwa 1.3
Mal pro Sekunde ($f_0 = \omega_0/2\pi$). Ohne Dämpfung würde das Fahrzeug nach
einer Bodenwelle endlos weiter wippen.

Aus der Vorlesung Mathematik 3 kennen wir die allgemeine Lösung

\begin{equation*}
x(t) = A \cos(\omega_0 t) + B \sin(\omega_0 t).
\end{equation*}

Mit den Anfangsbedingungen $x(0) = x_0$ und $\dot{x}(0) = 0$ folgern wir
...
von erster Ordnung und hat deshalb nur eine Integrationskonstante.
...
*Wie schreibt man `f(t, y)` für eine DGL zweiter Ordnung?* Wir übersetzen das
System von oben direkt in Python: `y[0]` ist die Position $x$, `y[1]` ist die
Geschwindigkeit $\dot{x}$. Die Funktion gibt beide Ableitungen zurück, genau wie
in Kapitel 10, nur mit zwei Einträgen statt nur einem Eintrag.
...
wir Dämpfung hinzu. Dann verliert das System Energie. Im Schwingfall mit
Dämpfung laufen die Phasenkurven spiralförmig in den Ursprung ein.
...
Eine DGL 2. Ordnung wie $m\ddot{x} + kx = 0$ wird durch die Substitution $y_1 =
x$, $y_2 = \dot{x}$ in ein System zweier Differentialgleichungen 1. Ordnung
umgeschrieben. `solve_ivp` löst das System ohne jede Anpassung: `y0` bekommt
zwei Einträge `[x0, v0]`, `f(t, y)` gibt zwei Werte zurück. `sol.y[0]` enthält
die Position, `sol.y[1]` die Geschwindigkeit. Die Eigenkreisfrequenz $\omega_0 =
\sqrt{k/m}$ bestimmt, wie schnell der ungedämpfte Schwinger schwingt. Das
Phasenporträt zeigt geschlossene Ellipsen, weil die mechanische Energie erhalten
bleibt.

In Abschnitt 11.3 erweitern wir das Modell um Dämpfung ($c\,\dot{x}$). Ohne
äußere Kraft laufen die Phasenkurven im gedämpften Schwingfall in den Ursprung,
