...
1\,\text{rad/s}$ statt $\omega_0 = \sqrt{k/m} = 8\,\text{rad/s}$. Der Integrator
hat dabei keine Fehler gemacht; er hat numerisch korrekt die falsch modellierte
DGL gelöst.
...
ax.set_title(f'Nichtlineares Pendel (T0 = {T_0:.3f} s)')
ax.legend()
plt.tight_layout()
plt.show()
