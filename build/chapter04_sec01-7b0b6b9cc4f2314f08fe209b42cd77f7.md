...
:class: tip
Die erste Code-Zelle mit `knoten_pos` und `lager_indizes` wurde bereits ausgeführt.


1. Lesen Sie nur aus dem Code ab, **ohne neue Ausgaben zu erzeugen**:  

   a) Welche Koordinaten hat Knoten 1 in Metern?  
   b) Welche Koordinaten hat Knoten 2 in Metern?


2. Formulieren Sie in einem kurzen Satz, was die Zeile  

   `lager_indizes = [0, 2]` **physikalisch** über das Fachwerk aussagt.
```

```{code-cell} python

### Optional: eigene Antworten kurz überprüfen

print("Knoten 1 (x, y) in m:", knoten_pos[:, 1])
print("Knoten 2 (x, y) in m:", knoten_pos[:, 2])
print("Lagerknoten-Indizes:", lager_indizes)
```
...
# Frage 1:
# knoten_pos[0, 1] = 1.0  -> x-Koordinate von Knoten 1
# knoten_pos[1, 1] = 1.0  -> y-Koordinate von Knoten 1
# Knoten 1 liegt also bei (1.0 m, 1.0 m).
#
# knoten_pos[0, 2] = 2.0  -> x-Koordinate von Knoten 2
# knoten_pos[1, 2] = 0.0  -> y-Koordinate von Knoten 2
# Knoten 2 liegt also bei (2.0 m, 0.0 m).

print("Knoten 1 (x, y) in m:", knoten_pos[:, 1])  # [1. 1.]
print("Knoten 2 (x, y) in m:", knoten_pos[:, 2])  # [2. 0.]

# Frage 2:
# lager_indizes = [0, 2] bedeutet:
# Knoten 0 und Knoten 2 sind fest gelagert und können sich nicht verschieben.
print("Lagerknoten-Indizes:", lager_indizes)
```
...
1. Knoten 1: $(1.0\,\text{m}, 1.0\,\text{m})$, Knoten 2: $(2.0\,\text{m}, 0.0\,\text{m})$.  
2. `lager_indizes = [0, 2]` bedeutet, dass Knoten 0 und Knoten 2 als Lager dienen und ihre Verschiebungen im Modell auf Null gesetzt werden.
