...
# Frage 1 und 2: verschiedene Schrittweiten
print(f"{'dx':>8}  {'Näherung':>12}  {'Fehler':>10}")
for dx in [0.1, 0.01, 0.001]:
    naeherung = vorwaerts(f, x0, dx)
    fehler    = abs(exakt - naeherung)
    print(f"{dx:>8.3f}  {naeherung:>12.8f}  {fehler:>10.2e}")

# Frage 3: andere Auswertestelle
print()
x1    = np.pi / 4
exakt_x1 = f_strich(x1)
print(f"Auswertestelle x0 = pi/4, analytisch: {exakt_x1:.8f}")
for dx in [0.1, 0.01, 0.001]:
    fehler = abs(exakt_x1 - vorwaerts(f, x1, dx))
    print(f"  dx = {dx:.3f}  Fehler = {fehler:.2e}")
```
...
print(f"{'dx':>12}  {'Vorwärts':>10}  {'Rückwärts':>10}  {'Zentral':>10}")
print("-" * 50)
for dx in schrittweiten:
    fv = abs(exakt - vorwaerts(f, x0, dx))
    fr = abs(exakt - rueckwaerts(f, x0, dx))
    fz = abs(exakt - zentral(f, x0, dx))
    print(f"{dx:>12.0e}  {fv:>10.2e}  {fr:>10.2e}  {fz:>10.2e}")

# Fragen 2 und 3: Log-Log-Plot
dx_werte  = np.logspace(-1, -10, 100)
fehler_v  = [abs(exakt - vorwaerts(f, x0, dx))   for dx in dx_werte]
fehler_r  = [abs(exakt - rueckwaerts(f, x0, dx)) for dx in dx_werte]
fehler_z  = [abs(exakt - zentral(f, x0, dx))     for dx in dx_werte]
